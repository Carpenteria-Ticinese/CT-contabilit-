...
async function newFattura(tipo) {
  ...
  applyIvaModeEditor()
  // FASE 27 — il termine si propone (nessun cliente ancora: vale l'aziendale),
  // e resta «non toccato» finche' Umberto non ci mette le mani.
  terminiFatturaToccati = false
  impostaTerminiFattura(giorniPropostiFattura(), null, true)
  aggiornaNotaTermine()
  renderRigheEditor()
}
...
async function editFattura(id) {
  ...
  try {
    ...
    // decisione gia' presa su quella fattura, e vince sulla proposta.
    _scadenzaBozzaAperta = f.data_scadenza || null
    ...
    // FASE 27 — dopo la data di emissione, perche' i giorni si contano da li'.
    terminiFatturaToccati = false
    impostaTerminiFattura(null, _scadenzaBozzaAperta, true)
    if (!_scadenzaBozzaAperta) impostaTerminiFattura(giorniPropostiFattura(), null, true)
    aggiornaNotaTermine()
...
// ═════════════════════════════════════════════════════════════════════════════
// FASE 27 — IL TERMINE DI PAGAMENTO E' DELLA FATTURA, NON DEL CLIENTE
//
// Fino a oggi i giorni arrivavano dalla scheda in rubrica e basta. Ma il
// termine e' una condizione DI QUEL DOCUMENTO: sullo stesso cliente si fanno
// trenta giorni di norma e pagamento anticipato su un acconto. Chi voleva
// cambiarlo doveva modificare la rubrica — cioe' cambiare le condizioni di
// tutte le fatture future per correggerne una.
//
// NON SI SALVA UN CAMPO NUOVO. Quello che finisce nel database e' la
// `data_scadenza`, che sulla fattura c'e' gia'. I giorni stampati sul
// documento si ricavano dalle due date, come dalla FASE 19: un dato solo, non
// due che un giorno diranno cose diverse.
//
// I DUE CAMPI SI TENGONO ALLINEATI: si scrivono i giorni e la data si
// aggiorna; si scrive la data e i giorni si aggiornano. Nessuno dei due e'
// «quello vero».
// ═════════════════════════════════════════════════════════════════════════════

// Vero quando Umberto ha messo le mani sul termine di questa fattura: da quel
// momento scegliere un altro cliente non glielo sovrascrive piu'.
var terminiFatturaToccati = false
// La scadenza letta dalla bozza che si sta aprendo: serve solo fra la lettura
// del documento e il riempimento dei campi.
var _scadenzaBozzaAperta = null

function segnaTerminiToccati() { terminiFatturaToccati = true }
...
function giorniPropostiFattura() {
  ...
  if (idContatto) {
    ...
    var g = c ? safeNum(c.giorni_pagamento) : null
    if (g != null) return g
  }
  var ga = safeNum((aziendaInfo || {}).termini_pagamento_giorni)
  return (ga != null) ? ga : 30
}

// Riempie i due campi. `forza` serve all'apertura di una bozza salvata: li'
// il valore arriva dal documento e vince su tutto.
function impostaTerminiFattura(giorni, scadenza, forza) {
  if (!el('f-fat-giorni')) return
  if (forza) terminiFatturaToccati = false
  if (scadenza) {
    el('f-fat-scadenza').value = scadenza
    var g = diffGiorni(scadenza, el('f-fat-data') ? el('f-fat-data').value : null)
    el('f-fat-giorni').value = (g == null) ? (giorni == null ? '' : giorni) : g
  } else {
    el('f-fat-giorni').value = (giorni == null) ? '' : giorni
    ricalcolaScadenzaDaGiorni()
  }
}

// Dai giorni alla data.
function ricalcolaScadenzaDaGiorni() {
  var dEm = el('f-fat-data') ? el('f-fat-data').value : ''
  var g = safeNum(el('f-fat-giorni') ? el('f-fat-giorni').value : '')
  if (!dEm || g == null || g < 0) return
  if (el('f-fat-scadenza')) el('f-fat-scadenza').value = addDays(dEm, Math.round(g))
  aggiornaNotaTermine()
}

function onGiorniFatturaChange() {
  segnaTerminiToccati()
  ricalcolaScadenzaDaGiorni()
}

// Dalla data ai giorni. Una scadenza prima dell'emissione darebbe giorni
// negativi: si lascia scrivere (capita di correggere la data dopo) ma il conto
// non si fa, cosi' non compare un «-3 giorni» sul documento.
function onScadenzaFatturaChange() {
  segnaTerminiToccati()
  var dEm = el('f-fat-data') ? el('f-fat-data').value : ''
  var dSc = el('f-fat-scadenza') ? el('f-fat-scadenza').value : ''
  if (dEm && dSc) {
    var g = diffGiorni(dSc, dEm)
    if (g != null && g >= 0 && el('f-fat-giorni')) el('f-fat-giorni').value = g
  }
  aggiornaNotaTermine()
}

// Cambiando la data di emissione la scadenza segue i giorni, non resta ferma.
function onDataEmissioneFatturaChange() { ricalcolaScadenzaDaGiorni() }

// La riga sotto il campo dice a parole cosa uscira' stampato: e' il posto in
// cui accorgersi che «0» non e' un errore ma una condizione.
function aggiornaNotaTermine() {
  var box = el('f-fat-scadenza-hint')
  if (!box) return
  var g = safeNum(el('f-fat-giorni') ? el('f-fat-giorni').value : '')
  var base = 'Puoi scrivere direttamente la data: i giorni qui accanto si aggiornano da soli.'
  if (g == null) { box.innerHTML = base; return }
  box.innerHTML = 'Sul documento si leggerà: <strong>' + esc(testoTerminePagamento(g)) +
                  '</strong>. ' + base
}

// ── COME SI SCRIVE UN TERMINE, in un posto solo ─────────────────────────────
// «0 giorni» sembra un difetto del programma, non una condizione commerciale.
// E «1 giorni» e' semplicemente sbagliato.
function testoTerminePagamento(giorni) {
  var g = safeNum(giorni)
  if (g == null) return '—'
  g = Math.round(g)
  if (g <= 0) return 'Pagamento alla consegna'
  return g === 1 ? '1 giorno' : g + ' giorni'
}

// Chiede se il termine appena usato deve diventare quello abituale del
// cliente. NON lo cambia da solo: una fattura con l'acconto anticipato non
// vuol dire che d'ora in poi quel cliente paghi sempre anticipato.
async function proponiTermineAbituale() {
  try {
    var idContatto = el('f-cli-contatto-id') ? el('f-cli-contatto-id').value : ''
    if (!idContatto) return
    var g = safeNum(el('f-fat-giorni') ? el('f-fat-giorni').value : '')
    if (g == null || g < 0) return
    g = Math.round(g)
    var c = (contattiCache || []).filter(function (x) { return x.id === idContatto })[0]
    if (!c) return
    var gCli = safeNum(c.giorni_pagamento)
    if (gCli != null && Math.round(gCli) === g) return          // gia' uguale
    var comeSta = (gCli == null)
      ? 'in rubrica non ha un termine suo'
      : 'in rubrica ha ' + testoTerminePagamento(gCli).toLowerCase()

    if (!window.confirm(
        'Su questa fattura hai messo: ' + testoTerminePagamento(g) + '.\n' +
        'Il cliente ' + contattoNome(c) + ' ' + comeSta + '.\n\n' +
        'Vuoi che ' + testoTerminePagamento(g).toLowerCase() +
        ' diventi il suo termine abituale?\n' +
        '(La fattura resta com’è in ogni caso.)')) return

    const { error } = await sb.from('tm_contatti')
      .update({ giorni_pagamento: g })
      .eq('id', idContatto).eq('azienda_id', currentAziendaId).select()
    if (error) throw error
    scadeCache('contatti')
  ...
  if (isNC) {
    ...
    payHtml =
      '<div class="inv-pay">' +
        ...
        // FASE 27 — «0 giorni» sembrava un errore del programma e «1 giorni»
        // era sbagliato: il testo lo decide testoTerminePagamento, in un posto
        // solo. Con zero giorni sparisce anche l'etichetta «Termine»: quello
        // che si legge e' la condizione, non un numero.
        (safeNum(giorni) != null && Math.round(safeNum(giorni)) <= 0
          // Etichetta «Pagamento» e valore «Alla consegna»: scrivere
          // «Pagamento: Pagamento alla consegna» sarebbe un'eco.
          ? '<div class="inv-pay-row"><span class="inv-pay-lbl">Pagamento</span><span>' +
              'Alla consegna</span></div>'
          : '<div class="inv-pay-row"><span class="inv-pay-lbl">Termine di pagamento</span><span>' +
              esc(testoTerminePagamento(giorni)) + '</span></div>') +
...
// ═════════════════════════════════════════════════════════════════════════════
// FASE 27 — NPA E LOCALITA': UNO COMPILA L'ALTRO
//
// DA DOVE ARRIVANO I DATI. Dall'elenco ufficiale svizzero delle localita' con
// NPA (Ortschaftenverzeichnis PLZ di swisstopo), scaricato una volta e
// INCORPORATO qui dentro. Nessuna chiamata a un servizio online: un campo che
// si compila solo quando la rete collabora e' peggio di un campo che non si
// compila, e con il logo l'abbiamo gia' pagata.
//
// QUANTO PESA. Tutta la Svizzera: 3.190 NPA, 58 KB di testo. Su un programma
// che ne pesa gia' 750 e' meno del dieci per cento, si scarica una volta sola
// e poi sta nella cache col ?v=. Il solo Ticino sarebbe stato 4 KB, ma Umberto
// fattura anche fuori cantone e un elenco parziale che tace su Zurigo sembra
// rotto. La STRINGA sta qui, ma le due mappe si costruiscono solo alla prima
// volta che si scrive in un campo NPA: chi non compila indirizzi non paga
// nemmeno quel lavoro.
//
// LE REGOLE, che contano piu' dei dati:
//   NPA -> localita'  se ce n'e' UNA sola la scrive; se sono piu' d'una le
//                     propone e fa scegliere. Non indovina.
//   localita' -> NPA  qui l'ambiguita' e' la norma (una citta' ha piu' NPA):
//                     propone soltanto, e scrive solo se il NPA e' uno solo.
//   MAI sovrascrivere quello che c'e' gia' scritto a mano: si riempie il
//                     vuoto, non si corregge il pieno. E' la stessa regola
//                     dell'indirizzo che arriva dal contatto.
//   NPA sconosciuto   non e' un errore: estero, o semplicemente non ancora
//                     scritto per intero. Si lascia scrivere, senza blocchi e
//                     senza avvisi rossi.
//   Paese != CH       il completamento sta zitto. Un NPA francese che somiglia
//                     a uno svizzero non deve tirar dentro una localita' del
//                     Ticino.
//
// FORMATO DELLA STRINGA: «NPA:localita,localita;NPA:localita;...»
// ═════════════════════════════════════════════════════════════════════════════

sur-Grandson,Grandevent;1422:Grandson;1423:Villars-Burquin,Fontanezier,Romairon,Vaugondry;1424:Champagne;1425:Onnens VD;1426:Concise,Corcelles-près-Concise;1427:Bonvillars;1428:Provence,Mutrux;1429:Giez;1430:Orges;1431:Vugelles-La Mothe,Novalles;1432:Belmont-sur-Yverdon,Gressy;1433:Suchy;1434:Ependes VD;1435:Essert-Pittet;1436:Treycovagnes,Chamblon;1437:Suscévaz;1438:Mathod;1439:Rances;1441:Valeyres-sous-Montagny;1442:Montagny-près-Yverdon;1443:Champvent,Essert-sous-Champvent,Villars-sous-Champvent;1445:Vuiteboeuf;1446:Baulmes;1450:Ste-Croix,La Sagne (Ste-Croix),Le Château-de-Ste-Croix;1452:Les Rasses;1453:Bullet,Mauborget;1454:L'Auberson,La Vraconnaz;1462:Yvonand;1463:Rovray;1464:Chavannes-le-Chêne,Chêne-Pâquier;1468:Cheyres;1470:Estavayer-le-Lac,Lully FR,Seiry,Bollion;1473:Font,Châtillon FR;1474:Châbles FR;1475:Autavaux,Forel FR,Montbrelloz;1482:Cugy FR;1483:Montet (Broye),Frasses,Vesin;1484:Aumont,Granges-de-Vesin;1485:Nuvilly;1486:Vuissens;1489:Murist;1509:Vucherens;1510:Moudon,Syens;1512:Chavannes-sur-Moudon;1513:Hermenches,Rossenges;1514:Bussy-sur-Moudon;1515:Villars-le-Comte,Neyruz-sur-Moudon;1521:Curtilles;1522:Lucens,Oulens-sur-Lucens;1523:Granges-près-Marnand;1524:Marnand;1525:Henniez,Seigneux;1526:Forel-sur-Lucens,Cremin;1527:Villeneuve FR;1528:Surpierre,Praratoud;1529:Cheiry;1530:Payerne;1532:Fétigny;1533:Ménières;1534:Sassel,Chapelle (Broye);1535:Combremont-le-Grand;1536:Combremont-le-Petit;1537:Champtauroz;1538:Treytorrens (Payerne);1541:Bussy FR,Morens FR,Sévaz;1542:Rueyres-les-Prés;1543:Grandcour;1544:Gletterens;1545:Chevroux;1551:Vers-chez-Perrin;1552:Trey;1553:Châtonnaye;1554:Sédeilles,Rossens VD;1555:Villarzel;1562:Corcelles-près-Payerne;1563:Dompierre FR;1564:Domdidier;1565:Missy,Vallon;1566:St-Aubin FR,Les Friques;1567:Delley;1568:Portalban;1580:Avenches,Oleyres,Donatyre;1583:Villarepos;1584:Villars-le-Grand;1585:Salavaux,Bellerive VD,Cotterd;1586:Vallamand;1587:Montmagny,Constantine;1588:Cudrefin;1589:Chabrey;1595:Faoug,Clavaleyres;1607:Palézieux,Palézieux-Village,Les Tavernes,Les Thioleyres;1608:Oron-le-Châtel,Bussigny-sur-Oron,Chesalles-sur-Oron,Chapelle (Glâne);1609:St-Martin FR,Besencens,Fiaugères;1610:Oron-la-Ville,Châtillens,Vuibroye;1611:Le Crêt-près-Semsales;1612:Ecoteaux;1613:Maracon;1614:Granges (Veveyse);1615:Bossonnens;1616:Attalens;1617:Remaufens,Tatroz;1618:Châtel-St-Denis;1619:Les Paccots;1623:Semsales;1624:La Verrerie,Grattavache,Progens;1625:Sâles (Gruyère),Maules;1626:Romanens,Rueyres-Treyfayes;1627:Vaulruz;1628:Vuadens;1630:Bulle;1632:Riaz;1633:Marsens,Vuippens;1634:La Roche FR;1635:La Tour-de-Trême;1636:Broc;1637:Charmey (Gruyère);1638:Morlon;1642:Sorens;1643:Gumefens;1644:Avry-devant-Pont;1645:Le Bry;1646:Echarlens;1647:Corbières;1648:Hauteville;1649:Pont-la-Ville;1651:Villarvolard;1652:Botterens,Villarbeney;1653:Crésuz,Châtel-sur-Montsalvens;1654:Cerniat FR;1656:Jaun,Im Fang;1657:Abländschen;1658:Rossinière,La Tine;1659:Rougemont,Flendruz;1660:Château-d'Oex,Les Moulins,L'Etivaz,La Lécherette;1661:Le Pâquier-Montbarry;1663:Gruyères,Pringy,Epagny,Moléson-sur-Gruyères;1665:Estavannens;1666:Grandvillard,Villars-sous-Mont;1667:Enney;1669:Albeuve,Lessoc,Les Sciernes-d'Albeuve,Neirivue,Montbovon;1670:Ursy,Bionnens,Esmonts;1673:Promasens,Gillarens,Auboranges,Ecublens FR,Rue;1674:Vuarmarens,Montet (Glâne),Morlens;1675:Vauderens,Blessens,Mossel;1676:Chavannes-les-Forts;1677:Prez-vers-Siviriez;1678:Siviriez;1679:Villaraboud;1680:Romont FR,Berlens;1681:Billens,Hennens;1682:Prévonloup,Lovatens,Dompierre VD,Villars-Bramard,Cerniaz VD;1683:Brenles,Chesalles-sur-Moudon,Sarzens;1684:Mézières FR;1685:Villariaz;1686:Grangettes-près-Romont,La Neirigue;1687:Vuisternens-devant-Romont,La Magne,Estévenens;1688:Sommentier,Lieffrens;1689:Le Châtelard-près-Romont;1690:Villaz-St-Pierre,Lussy FR;1691:Villarimboud;1692:Massonnens;1694:Villarsiviriaux,Villargiroud,Orsonnens,Chavannes-sous-Orsonnens;1695:Villarlod,Rueyres-St-Laurent,Estavayer-le-Gibloux,Villarsel-le-Gibloux;1696:Vuisternens-en-Ogoz;1697:La Joux FR,Les Ecasseys;1699:Porsel,Pont (Veveyse),Bouloz;1700:Fribourg;1712:Tafers;1713:St. Antoni;1714:Heitenried;1715:Alterswil FR;1716:Plaffeien,Oberschrot,Schwarzsee;1717:St. Ursen;1718:Rechthalten;1719:Brünisried,Zumholz;1720:Corminboeuf,Chésopelloz;1721:Misery,Cormérod,Courtion,Cournillens;1722:Bourguillon;1723:Marly,Villarsel-sur-Marly,Pierrafortscha;1724:Le Mouret,Essert FR,Ferpicloz,Montévraz,Oberried FR,Senèdes,Zénauva,Bonnefontaine;1725:Posieux;1726:Farvagny-le-Grand,Farvagny-le-Petit,Grenilles,Posat;1727:Corpataux,Magnedens;1728:Rossens FR;1730:Ecuvillens;1731:Ependes FR;1732:Arconciel;1733:Treyvaux;1734:Tentlingen;1735:Giffers;1736:St. Silvester;1737:Plasselb;1738:Sangernboden;1740:Neyruz FR;1741:Cottens FR;1742:Autigny;1744:Chénens;1745:Lentigny;1746:Prez-vers-Noréaz;1747:Corserey;1748:Torny-le-Grand;1749:Middes;1752:Villars-sur-Glâne;1753:Matran;1754:Corjolens,Avry-sur-Matran;1756:Onnens FR,Lovens;1757:Noréaz;1762:Givisiez;1763:Granges-Paccot;1772:Grolley,Nierlet-les-Bois,Ponthaux;1773:Léchelles,Chandon,Russy;1774:Cousset,Montagny-les-Monts;1775:Mannens,Grandsivaz;1776:Montagny-la-Ville;1782:Belfaux,Autafond,Lossy,La Corbaz,Cormagens,Formangueires;1783:Pensier,Barberêche;1784:Courtepin,Wallenried;1785:Cressier FR;1786:Sugiez;1787:Môtier (Vully),Mur (Vully) FR,Mur (Vully) VD;1788:Praz (Vully);1789:Lugnorre;1791:Courtaman;1792:Cordast,Guschelmuth;1793:Jeuss;1794:Salvenach;1795:Courlevon;1796:Courgevaux;1797:Münchenwiler;1800:Vevey;1801:Le Mont-Pèlerin;1802:Corseaux;1803:Chardonne;1804:Corsier-sur-Vevey;1805:Jongny;1806:St-Légier-La Chiésaz;1807:Blonay;1808:Les Monts-de-Corsier;1809:Fenil-sur-Corsier;1814:La Tour-de-Peilz;1815:Clarens;1816:Chailly-Montreux;1817:Brent;1820:Montreux,Territet,Veytaux;1822:Chernex;1823:Glion;1824:Caux;1832:Chamby,Villard-sur-Chamby;1833:Les Avants;1844:Villeneuve VD;1845:Noville;1846:Chessel;1847:Rennaz;1852:Roche VD;1853:Yvorne;1854:Leysin;1856:Corbeyrier;1860:Aigle;1862:Les Mosses,La Comballaz;1863:Le Sépey;1864:Vers-l'Eglise;1865:Les Diablerets;1866:La Forclaz VD;1867:Ollon VD,St-Triphon,Panex;1868:Collombey;1869:Massongex;1870:Monthey;1871:Choëx,Les Giettes;1872:Troistorrents;1873:Val-d'Illiez,Champoussin,Les Crosets;1874:Champéry;1875:Morgins;1880:Bex,Frenières-sur-Bex,Fenalet-sur-Bex,Les Plans-sur-Bex,Les Posses-sur-Bex;1882:Gryon;1884:Villars-sur-Ollon,Arveyes,Huémoz;1885:Chesières;1890:St-Maurice,Mex VS;1891:Vérossaz;1892:Lavey-Village,Lavey-les-Bains,Morcles;1893:Muraz (Collombey);1895:Vionnaz;1896:Vouvry,Miex;1897:Bouveret,Les Evouettes;1898:St-Gingolph;1899:Torgon;1902:Evionnaz;1903:Collonges;1904:Vernayaz;1905:Dorénaz;1906:Charrat;1907:Saxon;1908:Riddes;1911:Ovronnaz;1912:Leytron,Produit (Leytron),Montagnon (Leytron),Dugny (Leytron);1913:Saillon;1914:Isérables,Auddes-sur-Riddes;1918:La Tzoumaz;1920:Martigny;1921:Martigny-Croix;1922:Salvan,Les Granges (Salvan);1923:Les Marécottes,Le Trétien;1925:Finhaut,Le Châtelard VS;1926:Fully;1927:Chemin;1928:Ravoire;1929:Trient;1932:Bovernier,Les Valettes (Bovernier);1933:Sembrancher,Vens (Sembrancher),Chamoille (Sembrancher),La Garde (Sembrancher);1934:Le Châble VS,Bruson;1936:Verbier;1937:Orsières;1938:Champex-Lac;1941:Vollèges,Cries (Vollèges);1942:Levron;1943:Praz-de-Fort;1944:La Fouly VS;1945:Liddes,Fontaine Dessus (Liddes),Fontaine Dessous (Liddes),Dranse (Liddes),Chandonne (Liddes),Rive Haute (Liddes),Fornex (Liddes),Les Moulins VS (Liddes),Vichères (Liddes),Palasuit (Liddes),Chez Petit (Liddes),Petit Vichères (Liddes);1946:Bourg-St-Pierre;1947:Versegères,Champsec;1948:Lourtier,Fionnay,Sarreyer;1950:Sion;1955:Chamoson,Mayens-de-Chamoson,Les Vérines (Chamoson),Némiaz (Chamoson),Grugnay (Chamoson),St-Pierre-de-Clages;1957:Ardon;1958:St-Léonard,Uvrier;1961:Vernamiège;1962:Pont-de-la-Morge (Sion);1963:Vétroz;1964:Conthey;1965:Savièse;1966:Fortunau (Ayent),Luc (Ayent),St-Romain (Ayent),Saxonne (Ayent),Villa (Ayent),La Place (Ayent),Botyre (Ayent),Blignou (Ayent),Argnou (Ayent),Signèse (Ayent);1967:Bramois;1968:Mase;1969:St-Martin VS,Liez (St-Martin),Trogne (St-Martin),Suen (St-Martin),Eison (St-Martin);1971:Grimisuat,Champlan (Grimisuat);1972:Anzère;1973:Nax;1974:Arbaz;1975:St-Séverin;1976:Erde,Aven,Daillon;1977:Icogne;1978:Lens;1981:Vex;1982:Euseigne;1983:Evolène,Lanna;1984:Les Haudères,La Tour VS;1985:La Sage,La Forclaz VS,Villa (Evolène);1986:Arolla;1987:Hérémence;1988:Thyon,Les Collons;1991:Salins,Arvillard (Salins),Pravidondaz (Salins),Turin (Salins),Misériez (Salins);1992:Les Agettes,Les Mayens-de-Sion,La Vernaz (Les Agettes),Crête-à-l'Oeil (Les Agettes);1993:Veysonnaz,Clèbes (Nendaz);1994:Aproz (Nendaz);1996:Basse-Nendaz,Fey (Nendaz),Bieudron (Nendaz),Condémines (Nendaz),Saclentse,Baar (Nendaz),Beuson (Nendaz),Brignon (Nendaz);1997:Haute-Nendaz,Siviez (Nendaz),Sornard (Nendaz);2000:Neuchâtel;2012:Auvernier;2013:Colombier NE;2014:Bôle;2015:Areuse;2016:Cortaillod;2017:Boudry;2019:Rochefort,Chambrelien;2022:Bevaix;2023:Gorgier;2024:St-Aubin-Sauges;2025:Chez-le-Bart;2027:Montalchez,Fresens;2028:Vaumarcus;2034:Peseux;2035:Corcelles NE;2036:Cormondrèche;2037:Montmollin,Montezillon;2042:Valangin;2043:Boudevilliers;2046:Fontaines NE;2052:Fontainemelon,La Vue-des-Alpes;2053:Cernier;2054:Chézard-St-Martin,Les Vieux-Prés;2056:Dombresson;2057:Villiers;2058:Le Pâquier NE;2063:Vilars NE,Engollon,Fenin,Saules;2065:Savagnier;2067:Chaumont;2068:Hauterive NE;2072:St-Blaise;2073:Enges;2074:Marin-Epagnier;2075:Thielle,Wavre;2087:Cornaux NE;2088:Cressier NE;2103:Noiraigue;2105:Travers;2108:Couvet;2112:Môtiers NE;2113:Boveresse;2114:Fleurier;2115:Buttes;2116:Mont-de-Buttes;2117:La Côte
3172:Niederwangen b. Bern;3173:Oberwangen b. Bern;3174:Thörishaus;3175:Flamatt;3176:Neuenegg;3177:Laupen BE;3178:Bösingen;3179:Kriechenwil;3182:Ueberstorf;3183:Albligen;3184:Wünnewil;3185:Schmitten FR;3186:Düdingen;3202:Frauenkappelen;3203:Mühleberg;3204:Rosshäusern;3205:Gümmenen;3206:Rizenbach,Ferenbalm,Wallenbuch,Biberen,Gammen;3207:Wileroltigen,Golaten;3208:Gurbrü;3210:Kerzers;3212:Gurmels,Kleingurmels;3213:Liebistorf,Kleinbösingen;3214:Ulmiz;3215:Gempenach,Büchslen,Lurtigen;3216:Ried b. Kerzers,Agriswil;3225:Müntschemier;3226:Treiten;3232:Ins;3233:Tschugg;3234:Vinelz;3235:Erlach;3236:Gampelen;3237:Brüttelen;3238:Gals;3250:Lyss;3251:Wengi b. Büren,Ruppoldsried;3252:Worben;3253:Schnottwil;3254:Messen,Balm b. Messen;3255:Rapperswil BE;3256:Dieterswil,Bangerten b. Dieterswil,Seewil;3257:Grossaffoltern,Ammerzwil BE;3262:Suberg;3263:Büetigen;3264:Diessbach b. Büren;3266:Wiler b. Seedorf;3267:Seedorf BE;3268:Lobsigen;3270:Aarberg;3271:Radelfingen b. Aarberg;3272:Walperswil,Epsach;3273:Kappelen;3274:Hermrigen,Bühl b. Aarberg,Merzligen;3280:Murten,Meyriez,Greng;3282:Bargen BE;3283:Kallnach,Niederried b. Kallnach;3284:Fräschels;3285:Galmiz;3286:Muntelier;3292:Busswil BE;3293:Dotzigen;3294:Büren an der Aare,Meienried;3295:Rüti b. Büren;3296:Arch;3297:Leuzigen;3298:Oberwil b. Büren;3302:Moosseedorf;3303:Jegenstorf,Münchringen,Zuzwil BE,Ballmoos;3305:Iffwil,Scheunen;3306:Etzelkofen;3307:Brunnenthal;3308:Grafenried;3309:Kernenried,Zauggenried;3312:Fraubrunnen;3313:Büren zum Hof;3314:Schalunen;3315:Bätterkinden,Kräiligen;3317:Limpach,Mülchi;3322:Urtenen-Schönbühl,Mattstetten;3323:Bäriswil BE;3324:Hindelbank,Mötschwil;3325:Hettiswil b. Hindelbank;3326:Krauchthal;3360:Herzogenbuchsee;3362:Niederönz;3363:Oberönz;3365:Grasswil,Seeberg;3366:Bettenhausen,Bollodingen;3367:Thörigen,Ochlenberg;3368:Bleienbach;3372:Wanzwil;3373:Röthenbach Herzogenbuchsee,Heimenhausen;3374:Wangenried;3375:Inkwil;3376:Graben,Berken;3377:Walliswil b. Wangen;3380:Wangen an der Aare,Walliswil b. Niederbipp;3400:Burgdorf;3412:Heimiswil;3413:Kaltacker;3414:Oberburg;3415:Hasle b. Burgdorf,Schafhausen im Emmental,Rüegsauschachen;3416:Affoltern im Emmental;3417:Rüegsau;3418:Rüegsbach;3419:Biembach im Emmental;3421:Lyssach,Rüti b. Lyssach;3422:Kirchberg BE,Rüdtligen,Alchenflüh;3423:Ersigen;3424:Niederösch,Oberösch;3425:Koppigen,Willadingen;3426:Aefligen;3427:Utzenstorf;3428:Wiler b. Utzenstorf;3429:Höchstetten,Hellsau;3432:Lützelflüh-Goldbach;3433:Schwanden im Emmental;3434:Obergoldbach,Landiswil;3435:Ramsei;3436:Zollbrück;3437:Rüderswil;3438:Lauperswil;3439:Ranflüh;3452:Grünenmatt;3453:Heimisbach;3454:Sumiswald;3455:Grünen;3456:Trachselwald;3457:Wasen im Emmental;3462:Weier im Emmental;3463:Häusernmoos im Emmental;3464:Schmidigen-Mühleweg;3465:Dürrenroth;3472:Wynigen,Rumendingen;3473:Alchenstorf;3474:Rüedisbach;3475:Riedtwil,Hermiswil;3476:Oschwand;3503:Gysenstein;3504:Niederhünigen,Oberhünigen;3506:Grosshöchstetten;3507:Biglen;3508:Arni BE;3510:Konolfingen,Freimettigen,Häutligen;3512:Walkringen;3513:Bigenthal;3531:Oberthal;3532:Zäziwil,Mirchel;3533:Bowil;3534:Signau;3535:Schüpbach;3536:Aeschau;3537:Eggiwil;3538:Röthenbach im Emmental;3543:Emmenmatt;3550:Langnau im Emmental;3551:Oberfrittenbach;3552:Bärau;3553:Gohl;3555:Trubschachen;3556:Trub;3557:Fankhaus (Trub);3600:Thun;3603:Thun;3604:Thun;3608:Thun;3612:Steffisburg;3613:Steffisburg;3614:Unterlangenegg;3615:Heimenschwand;3616:Schwarzenegg;3617:Fahrni b. Thun;3618:Süderen;3619:Eriz;3622:Homberg b. Thun;3623:Teuffenthal b. Thun,Horrenbach,Buchen BE;3624:Goldiwil (Thun),Schwendibach;3625:Heiligenschwendi;3626:Hünibach;3627:Heimberg;3628:Uttigen;3629:Kiesen,Oppligen,Jaberg;3631:Höfen b. Thun;3632:Niederstocken,Oberstocken;3633:Amsoldingen;3634:Thierachern;3635:Uebeschi;3636:Längenbühl,Forst b. Längenbühl;3638:Blumenstein,Pohlern;3645:Gwatt (Thun),Zwieselberg;3646:Einigen;3647:Reutigen;3652:Hilterfingen;3653:Oberhofen am Thunersee;3654:Gunten;3655:Sigriswil;3656:Tschingel ob Gunten,Aeschlen ob Gunten,Ringoldswil;3657:Schwanden (Sigriswil);3658:Merligen;3661:Uetendorf;3662:Seftigen;3663:Gurzelen;3664:Burgistein;3665:Wattenwil;3671:Brenzikofen,Herbligen;3672:Oberdiessbach,Aeschlen b. Oberdiessbach;3673:Linden;3674:Bleiken b. Oberdiessbach;3700:Spiez;3702:Hondrich;3703:Aeschi b. Spiez,Aeschiried;3704:Krattigen;3705:Faulensee;3706:Leissigen;3707:Därligen;3711:Emdthal,Mülenen;3713:Reichenbach im Kandertal;3714:Frutigen,Wengi b. Frutigen;3715:Adelboden;3716:Kandergrund;3717:Blausee-Mitholz;3718:Kandersteg;3722:Scharnachtal;3723:Kiental;3724:Ried (Frutigen);3725:Achseten;3752:Wimmis;3753:Oey;3754:Diemtigen;3755:Horboden;3756:Zwischenflüh;3757:Schwenden im Diemtigtal;3758:Latterbach;3762:Erlenbach im Simmental;3763:Därstetten;3764:Weissenburg;3765:Oberwil im Simmental;3766:Boltigen;3770:Zweisimmen;3771:Blankenburg;3772:St. Stephan;3773:Matten (St. Stephan);3775:Lenk im Simmental;3776:Oeschseite;3777:Saanenmöser;3778:Schönried;3780:Gstaad;3781:Turbach;3782:Lauenen b. Gstaad;3783:Grund b. Gstaad;3784:Feutersoey;3785:Gsteig b. Gstaad;3792:Saanen;3800:Interlaken,Unterseen,Matten b. Interlaken,Sundlauenen;3801:Jungfraujoch;3803:Beatenberg;3804:Habkern;3805:Goldswil b. Interlaken;3806:Bönigen b. Interlaken;3807:Iseltwald;3812:Wilderswil;3813:Saxeten;3814:Gsteigwiler;3815:Zweilütschinen,Gündlischwand;3816:Lütschental,Burglauenen;3818:Grindelwald;3822:Lauterbrunnen,Isenfluh;3823:Wengen,Kleine Scheidegg,Eigergletscher;3824:Stechelberg;3825:Mürren;3826:Gimmelwald;3852:Ringgenberg BE;3853:Niederried b. Interlaken;3854:Oberried am Brienzersee;3855:Brienz BE,Axalp,Schwanden b. Brienz;3856:Brienzwiler;3857:Unterbach BE;3858:Hofstetten b. Brienz;3860:Meiringen,Rosenlaui,Schattenhalb,Brünig;3862:Innertkirchen;3863:Gadmen;3864:Guttannen;3900:Brig,Gamsen,Brigerbad;3901:Rothwald;3902:Glis;3903:Mund,Birgisch;3904:Naters;3905:Saas-Almagell;3906:Saas-Fee;3907:Simplon Dorf,Simplon Hospiz,Gabi (Simplon),Gondo;3908:Saas-Balen;3910:Saas-Grund;3911:Ried-Brig;3912:Termen;3913:Rosswald;3914:Blatten b. Naters,Belalp;3916:Ferden;3917:Kippel,Goppenstein;3918:Wiler (Lötschen);3919:Blatten (Lötschen);3920:Zermatt;3922:Stalden VS,Kalpetran,Eisten;3923:Törbel;3924:St. Niklaus VS;3925:Grächen;3926:Embd;3927:Herbriggen;3928:Randa;3929:Täsch;3930:Visp,Eyholz;3931:Lalden;3932:Visperterminen;3933:Staldenried;3934:Zeneggen;3935:Bürchen;3937:Baltschieder;3938:Ausserberg;3939:Eggerberg;3940:Steg VS;3942:Raron,Niedergesteln,St. German;3943:Eischoll;3944:Unterbäch VS;3945:Gampel,Niedergampel;3946:Turtmann,Gruben;3947:Ergisch;3948:Unterems,Oberems;3949:Hohtenn;3951:Agarn;3952:Susten;3953:Leuk Stadt,Inden,Varen;3954:Leukerbad;3955:Albinen;3956:Guttet-Feschel;3957:Erschmatt,Bratsch;3960:Sierre,Corin-de-la-Crête,Loc,Niouc,Muraz (Sierre);3961:Vissoie,St-Luc,Chandolin,Ayer,Zinal,Grimentz,Mission,St-Jean VS;3963:Crans-Montana,Montana,Aminona,Crans VS;3965:Chippis;3966:Chalais,Réchy;3967:Vercorin;3968:Veyras;3970:Salgesch;3971:Chermignon,Chermignon-d'en-Bas,Ollon VS;3972:Miège;3973:Venthône;3974:Mollens VS;3975:Randogne;3976:Noës,Champzabé;3977:Granges VS;3978:Flanthey;3979:Grône;3982:Bitsch;3983:Mörel,Bister,Filet,Goppisberg,Greich;3984:Fies
steg;6475:Bristen;6476:Intschi;6482:Gurtnellen;6484:Wassen UR;6485:Meien;6487:Göschenen;6490:Andermatt;6491:Realp;6493:Hospental;6500:Bellinzona;6503:Bellinzona;6512:Giubiasco;6513:Monte Carasso;6514:Sementina;6515:Gudo;6516:Cugnasco;6517:Arbedo;6518:Gorduno;6523:Preonzo;6524:Moleno;6525:Gnosca;6526:Prosito;6527:Lodrino;6528:Camorino;6532:Castione;6533:Lumino;6534:S. Vittore;6535:Roveredo GR;6537:Grono;6538:Verdabbio;6540:Castaneda;6541:Sta. Maria in Calanca;6542:Buseno;6543:Arvigo;6544:Braggio;6545:Selma;6546:Cauco;6548:Rossa,Augio,Sta. Domenica;6549:Laura;6556:Leggia;6557:Cama;6558:Lostallo;6562:Soazza;6563:Mesocco;6565:S. Bernardino;6571:Indemini;6572:Quartino;6573:Magadino;6574:Vira (Gambarogno);6575:S. Nazzaro,Vairano;6576:Gerra (Gambarogno);6577:Ranzo;6578:Caviano;6579:Piazzogna;6582:Pianezzo;6583:S. Antonio (Val Morobbia);6584:Carena;6592:S. Antonino;6593:Cadenazzo;6594:Contone;6595:Riazzino;6596:Gordola;6597:Agarone;6598:Tenero;6599:Robasacco;6600:Locarno,Muralto,Solduno;6605:Locarno;6612:Ascona;6613:Porto Ronco;6614:Brissago,Isole di Brissago;6616:Losone;6618:Arcegno;6622:Ronco sopra Ascona;6631:Corippo;6632:Vogorno;6633:Lavertezzo;6634:Brione (Verzasca);6635:Gerra (Verzasca);6636:Frasco;6637:Sonogno;6644:Orselina;6645:Brione sopra Minusio;6646:Contra;6647:Mergoscia;6648:Minusio;6652:Tegna;6653:Verscio;6654:Cavigliano;6655:Intragna,Verdasio,Rasa;6656:Golino;6657:Palagnedra;6658:Borgnone;6659:Camedo,Moneto;6661:Loco,Auressio,Berzona,Mosogno,Russo,Comologno,Spruga,Crana,Vergeletto,Gresso;6670:Avegno;6672:Gordevio;6673:Maggia;6674:Someo,Riveo;6675:Cevio;6676:Bignasco;6677:Aurigeno,Moghegno;6678:Giumaglio,Lodano,Coglio;6682:Linescio;6683:Cerentino,Niva (Vallemaggia);6684:Campo (Vallemaggia),Cimalmotto;6685:Bosco/Gurin;6690:Cavergno,S. Carlo (Val Bavona);6692:Menzonio,Brontallo;6693:Broglio;6694:Prato-Sornico;6695:Peccia,Piano di Peccia;6696:Fusio;6702:Claro;6703:Osogna;6705:Cresciano;6707:Iragna;6710:Biasca;6713:Malvaglia;6714:Semione;6715:Dongio;6716:Acquarossa,Leontica,Lottigna;6717:Dangio,Torre;6718:Olivone,Camperio;6719:Aquila;6720:Campo (Blenio),Ghirone;6721:Ludiano,Motto (Blenio);6722:Corzoneso;6723:Prugiasco,Castro,Marolta;6724:Ponto Valentino,Largario;6742:Pollegio;6743:Bodio TI;6744:Personico;6745:Giornico;6746:Lavorgo,Calonico,Nivo;6747:Chironico;6748:Anzonico;6749:Sobrio,Cavagnago;6760:Faido,Molare,Calpiogna,Campello,Carì,
(Opfikon);8153:Rümlang;8154:Oberglatt ZH;8155:Niederhasli,Nassenwil;8156:Oberhasli;8157:Dielsdorf;8158:Regensberg;8162:Steinmaur,Sünikon;8164:Bachs;8165:Schöfflisdorf,Oberweningen,Schleinikon;8166:Niederweningen;8172:Niederglatt ZH;8173:Neerach;8174:Stadel b. Niederglatt;8175:Windlach;8180:Bülach;8181:Höri;8182:Hochfelden;8184:Bachenbülach;8185:Winkel;8187:Weiach;8192:Glattfelden,Zweidlen;8193:Eglisau;8194:Hüntwangen;8195:Wasterkingen;8196:Wil ZH;8197:Rafz;8200:Schaffhausen;8203:Schaffhausen;8207:Schaffhausen;8208:Schaffhausen;8212:Neuhausen am Rheinfall,Nohl;8213:Neunkirch;8214:Gächlingen;8215:Hallau;8216:Oberhallau;8217:Wilchingen;8218:Osterfingen;8219:Trasadingen;8222:Beringen;8223:Guntmadingen;8224:Löhningen;8225:Siblingen;8226:Schleitheim;8228:Beggingen;8231:Hemmental;8232:Merishausen;8233:Bargen SH;8234:Stetten SH;8235:Lohn SH;8236:Büttenhardt,Opfertshofen SH;8239:Dörflingen;8240:Thayngen;8241:Barzheim;8242:Bibern SH,Hofen SH;8243:Altdorf SH;8245:Feuerthalen;8246:Langwiesen;8247:Flurlingen;8248:Uhwiesen;8252:Schlatt TG;8253:Diessenhofen,Willisdorf;8254:Basadingen;8255:Schlattingen;8259:Kaltenbach,Etzwilen,Rheinklingen,Wagenhausen;8260:Stein am Rhein;8261:Hemishofen;8262:Ramsen;8263:Buch SH;8264:Eschenz;8265:Mammern;8266:Steckborn;8267:Berlingen;8268:Mannenbach-Salenstein,Salenstein;8269:Fruthwilen;8272:Ermatingen;8273:Triboltingen;8274:Tägerwilen,Gottlieben;8280:Kreuzlingen;8302:Kloten;8303:Bassersdorf;8304:Wallisellen;8305:Dietlikon;8306:Brüttisellen;8307:Effretikon,Ottikon b. Kemptthal;8308:Illnau,Agasul;8309:Nürensdorf;8310:Kemptthal,Grafstal;8311:Brütten;8312:Winterberg ZH;8314:Kyburg;8315:Lindau;8317:Tagelswangen;8320:Fehraltorf;8322:Madetswil,Gündisau;8330:Pfäffikon ZH;8331:Auslikon;8332:Russikon,Rumlikon;8335:Hittnau;8340:Hinwil;8342:Wernetshausen;8344:Bäretswil;8345:Adetswil;8352:Elsau,Ricketwil (Winterthur);8353:Elgg;8354:Hofstetten ZH,Dickbuch;8355:Aadorf;8356:Ettenhausen TG;8357:Guntershausen b. Aadorf;8360:Eschlikon TG,Wallenwil;8362:Balterswil;8363:Bichelsee;8370:Sirnach;8371:Busswil TG;8372:Wiezikon b. Sirnach;8374:Dussnang,Oberwangen TG;8376:Fischingen,Au TG;8400:Winterthur;8404:Winterthur,Reutlingen (Winterthur),Stadel (Winterthur);8405:Winterthur;8406:Winterthur;8408:Winterthur;8409:Winterthur;8412:Aesch (Neftenbach),Riet (Neftenbach),Hünikon (Neftenbach);8413:Neftenbach;8414:Buch am Irchel;8415:Berg am Irchel,Gräslikon;8416:Flaach;8418:Schlatt ZH;8421:Dättlikon;8422:Pfungen;8424:Embrach;8425:Oberembrach;8426:Lufingen;8427:Freienstein,Rorbas;8428:Teufen ZH;8442:Hettlingen;8444:Henggart;8447:Dachsen;8450:Andelfingen;8451:Kleinandelfingen;8452:Adlikon b. Andelfingen;8453:Alten;8454:Buchberg;8455:Rüdlingen;8457:Humlikon;8458:Dorf;8459:Volken;8460:Marthalen;8461:Oerlingen;8462:Rheinau;8463:Benken ZH;8464:Ellikon am Rhein;8465:Rudolfingen,Wildensbuch;8466:Trüllikon;8467:Truttikon;8468:Waltalingen,Guntalingen;8471:Rutschwil (Dägerlen),Dägerlen,Oberwil (Dägerlen),Berg (Dägerlen),Bänk (Dägerlen);8472:Seuzach,Unterohringen,Oberohringen;8474:Dinhard;8475:Ossingen;8476:Unterstammheim;8477:Oberstammheim;8478:Thalheim an der Thur;8479:Altikon;8482:Sennhof (Winterthur);8483:Kollbrunn;8484:Weisslingen,Neschwil,Theilingen;8486:Rikon im Tösstal;8487:Zell ZH,Rämismühle;8488:Turbenthal;8489:Wildberg,Schalchen,Ehrikon;8492:Wila;8493:Saland;8494:Bauma;8495:Schmidrüti;8496:Steg im Tösstal;8497:Fischenthal;8498:Gibswil;8499:Sternenberg;8500:Frauenfeld,Gerlikon;8505:Pfyn,Dettighofen;8506:Lanzenneunforn;8507:Hörhausen;8508:Homburg;8512:Thundorf,Lustdorf,Wetzikon TG;8514:Amlikon-Bissegg;8522:Häuslenen,Aawangen;8523:Hagenbuch ZH;8524:Uesslingen,Buch b. Frauenfeld;8525:Niederneunforn,Wilen b. Neunforn;8526:Oberneunforn;8532:Warth,Weiningen TG;8535:Herdern;8536:Hüttwilen;8537:Nussbaumen TG,Uerschhausen;8542:Wiesendangen;8543:Bertschikon,Gundetswil,Kefikon ZH;8544:Attikon;8545:Rickenbach ZH,Rickenbach Sulz;8546:Islikon,Kefikon TG,Menzengrüt;8547:Gachnang;8548:Ellikon an der Thur;8552:Felben-Wellhausen;8553:Harenwilen,Mettendorf TG,Hüttlingen,Eschikofen;8554:Müllheim-Wigoltingen,Bonau;8555:Müllheim Dorf;8556:Wigoltingen,Engwang,Illhart,Lamperswil TG;8558:Raperswilen;8560:Märstetten;8561:Ottoberg;8564:Hefenhausen,Engwilen,Sonterswil,Wäldi,Lipperswil,Wagerswil,Hattenhausen,Gunterswilen;8565:Hugelshofen;8566:Neuwilen,Dotnacht,Ellighausen,Lippoldswilen;8570:Weinfelden;8572:Berg TG,Andhausen,Graltshausen,Guntershausen b. Berg;8573:Siegershausen,Alterswilen,Altishausen;8574:Illighausen,Oberhofen TG,Lengwil,Dettighofen (Lengwil);8575:Bürglen TG,Istighofen;8576:Mauren TG;8577:Schönholzerswilen;8580:Amriswil,Sommeri,Hagenwil b. Amriswil,Hefenhofen,Biessenhofen;8581:Schocherswil;8582:Dozwil;8583:Sulgen,Götighofen,Donzhausen;8584:Leimbach TG,Opfershofen TG;8585:Mattwil,Happerswil,Langrickenbach,Zuben,Schönenbaumgarten,Herrenhof,Birwinken,Klarsreuti;8586:Erlen,Andwil TG,Kümmertshausen,Riedt b. Erlen,Buchackern,Engishofen,Ennetaach,Buch b. Kümmertshausen;8587:Oberaach;8588:Zihlschlacht;8589:Sitterdorf;8590:Romanshorn;8592:Uttwil;8593:Kesswil;8594:Güttingen;8595:Altnau;8596:Scherzingen,Münsterlingen;8597:Landschlacht;8598:Bottighofen;8599:Salmsach;8600:Dübendorf;8602:Wangen b. Dübendorf;8603:Schwerzenbach;8604:Volketswil;8605:Gutenswil;8606:Nänikon,Greifensee;8607:Aathal-Seegräben;8608:Bubikon;8610:Uster;8614:Bertschikon (Gossau ZH),Sulzbach;8615:Wermatswil,Freudwil;8616:Riedikon;8617:Mönchaltorf;8618:Oetwil am See;8620:Wetzikon ZH;8623:Wetzikon ZH;8624:Grüt (Gossau ZH);8625:Gossau ZH;8626:Ottikon (Gossau ZH);8627:Grüningen;8630:Rüti ZH;8632:Tann;8633:Wolfhausen;8634:Hombrechtikon;8635:Dürnten;8636:Wald ZH;8637:Laupen ZH;8638:Goldingen;8640:Rapperswil SG,Hurden;8645:Jona;8646:Wagen;8700:Küsnacht ZH;8702:Zollikon;8703:Erlenbach ZH;8704:Herrliberg;8706:Meilen;8707:Uetikon am See;8708:Männedorf;8712:Stäfa;8713:Uerikon;8714:Feldbach;8715:Bollingen;8716:Schmerikon;8717:Benken SG;8718:Schänis;8722:Kaltbrunn;8723:Rufi,Maseltrangen;8725:Ernetschwil,Gebertingen;8726:Ricken SG;8727:Walde SG;8730:Uznach;8732:Neuhaus SG;8733:Eschenbach SG;8734:Ermenswil;8735:St. Gallenkappel,Rüeterswil;8737:Gommiswald;8738:Uetliburg SG;8739:Rieden SG;8750:Glarus,Riedern,Klöntal;8751:Urnerboden;8752:Näfels;8753:Mollis;8754:Netstal;8755:Ennenda;8756:Mitlödi;8757:Filzbach;8758:Obstalden;8762:Schwanden GL,Schwändi b. Schwanden,Sool;8765:Engi;8766:Matt;8767:Elm;8772:Nidfurn;8773:Haslen GL;8774:Leuggelbach;8775:Luchsingen,Hätzingen;8777:Diesbach GL,Betschwanden;8782:Rüti GL;8783:Linthal;8784:Braunwald;8800:Thalwil;8802:Kilchberg ZH;8803:Rüschlikon;8804:Au ZH;8805:Richterswil;8806:Bäch SZ;8807:Freienbach;8808:Pfäffikon SZ;8810:Horgen;8815:Horgenberg;8816:Hirzel;8820:Wädenswil;8824:Schönenberg ZH;8825:Hütten;8832:Wollerau,Wilen b. Wollerau;8833:Samstagern;8834:Schindellegi;8835:Feusisberg;8836:Bennau;8840:Einsiedeln,Trachslau;8841:Gross;8842:Unteriberg;8843:Oberiberg;8844:Euthal;8845:Studen SZ;8846:Willerzell;8847:Egg SZ;8849:Alpthal;8852:Altendorf;8853:Lachen SZ;8854:Siebnen,Galgenen;8855:Wangen SZ;8856:Tuggen;8857:Vorderthal;8858:Innerthal;8862:Schübelbach;8863:Buttikon SZ;8864:Reichenburg;8865:Bilten;8866:Ziegelbrücke;8867:Niederurnen;8868:Oberurnen;8872:Weesen;8873:Amden;8874:Mühlehorn;8877:Murg;8878:Quinten;8880:Walenstadt;8881:Walenstadtberg,Tscherlach;8882:Unterterzen;8883:Quarten;8884:Oberterzen;8885:Mols;8886:Mädris-Vermol;8887:Mels;8888:Heiligkreuz (Mels);8889:Plons;8890:Flums;8892:Berschis;8893:Flums Hochwiese;8894:Flumserberg Saxli;8895:Flumserberg Portels;8896:Flumserberg Bergheim;8897:Flumserberg Tannenheim;8898:Flumserberg Tannenbodenalp;8902:Urdorf;8903:Birmensdorf ZH;8904:Aesch ZH;8905:Islisberg,Arni AG;8906:Bonstetten;8907:Wettswil;8908:Hedingen;8909:Zwillikon;8910:Affoltern am Albis;8911:Rifferswil;8912:Obfelden;8913:Ottenbach;8914:Aeugst am Albis,Aeugstertal;8915:Hausen am Albis;8916:Jonen;8917:Oberlunkhofen;8918:Unterlunkhofen;8919:Rottenschwil;8925:Ebertswil;8926:Kappel am Albis,Hauptikon,Uerzlikon;8932:Mettmenstetten;8933:Maschwanden;8934:Knonau;8942:Oberrieden;8951:Fahrweid;8952:Schlieren;8953:Dietikon;8954:Geroldswil;8955:Oetwil an der Limmat;8956:Killwangen;8957:Spreitenbach;8962:Bergdietikon;8964:Rudolfstetten;8965:Berikon;8966:Oberwil-Lieli;8967:Widen;9000:St. Gallen;9008:St. Gallen;9010:St. Gallen;9011:St. Gallen;9012:St. Gallen;9014:St. Gallen;9015:St. Gallen;9016:St. Gallen;9030:Abtwil SG,St. Josefen;9032:Engelburg;9033:Untereggen;9034:Eggersriet;9035:Grub AR;9036:Grub SG;9037:Speicherschwendi;9038:Rehetobel;9042:Speicher;9043:Trogen;9044:Wald AR;9050:Appenzell,Appenzell Eggerstanden,Appenzell Enggenhütten,Appenzell Meistersrüte,Appenzell Steinegg,Appenzell Schlatt;9052:Niederteufen;9053:Teufen AR;9054:Haslen AI;9055:Bühler;9056:Gais;9057:Weissbad,Schwende,Wasserauen;9058:Brülisau;9062:Lustmühle;9063:Stein AR;9064:Hundwil;9100:Herisau;9103:Schwellbrunn;9104:Waldstatt;9105:Schönengrund;9107:Urnäsch;9108:Gonten,Gontenbad,Jakobsbad;9112:Schachen b. Herisau;9113:Degersheim;9114:Hoffeld;9115:Dicken;9116:Wolfertswil;9122:Mogelsberg,Ebersol;9123:Nassen;9125:Brunnadern;9126:Necker;9127:St. Peterzell;9200:Gossau SG;9203:Niederwil SG;9204:Andwil SG;9205:Waldkirch;9212:Arnegg;9213:Hauptwil;9214:Kradolf;9215:Schönenberg an der Thur,Buhwil;9216:Heldswil,Hohentannen;9217:Neukirch an der Thur;9220:Bischofszell;9223:Schweizersholz,Halden;9225:Wilen (Gottshaus),St. Pelagiberg;9230:Flawil;9231:Egg (Flawil);9240:Uzwil,Niederglatt SG;9242:Oberuzwil;9243:Jonschwil;9244:Niederuzwil;9245:Oberbüren,Sonnental;9246:Niederbüren;9247:Henau;9248:Bichwil;9249:Algetshausen,Niederstetten,Oberstetten;9300:Wittenbach;9304:Bernhardzell;9305:Berg SG;9306:Freidorf TG;9308:Lömmenschwil;9312:Häggenschwil;9313:Muolen;9314:Steinebrunn;9315:Neukirch (Egnach),Winden;9320:Arbon,Frasnacht,Stachen;9322:Egnach;9323:Steinach;9325:Roggwil TG;9326:Horn;9327:Tübach;9400:Rorschach;9402:Mörschwil;9403:Goldach;9404:Rorschacherberg;9405:Wienacht-Tobel;9410:Heiden;9411:Reute AR,Schachen b. Reute;9413:Oberegg;9422:Staad SG;9423:Altenrhein;9424:Rheineck;9425:Thal;9426:Lutzenberg;9427:Wolfhalden;9428:Walzenhausen;9430:St. Margrethen SG;9434:Au SG;9435:Heerbrugg;9436:Balgach;9437:Marbach SG;9442:Berneck,Büriswilen;9443:Widnau;9444:Diepoldsau;9445:Rebstein;9450:Altstätten SG,Lüchingen;9451:Kriessern;9452:Hinterforst;9453:Eichberg;9462:Montlingen;9463:Oberriet SG;9464:Rüthi (Rheintal),Lienz;9465:Salez;9466:Sennwald;9467:Frümsen;9468:Sax;9469:Haag (Rheintal);9470:Buchs SG,Werdenberg;9472:Grabs,Grabserberg;9473:Gams;9475:Sevelen;9476:Weite,Fontnas;9477:Trübbach;9478:Azmoos;9479:Oberschan,Malans SG,Gretschins;9485:Nendeln;9486:Schaanwald;9487:Gamprin-Bendern;9488:Schellenberg;9490:Vaduz;9491:Ruggell;9492:Eschen;9493:Mauren FL;9494:Schaan;9495:Triesen;9496:Balzers;9497:Triesenberg;9498:Planken;9500:Wil SG;9502:Braunau;9503:Stehrenberg,Lanterswil;9504:Friltschen;9506:Lommis;9507:Stettfurt;9508:Weingarten-Kalthäusern;9512:Rossrüti;9514:Wuppenau;9515:Hosenruck;9517:Mettlen;9523:Züberwangen;9524:Zuzwil SG;9525:Lenggenwil;9526:Zuckenriet;9527:Niederhelfenschwil;9532:Rickenbach b. Wil;9533:Kirchberg SG,Dietschwil;9534:Gähwil;9535:Wilen b. Wil;9536:Schwarzenbach SG;9542:Münchwilen TG;9543:St. Margarethen TG;9545:Wängi;9546:Tuttwil;9547:Wittenwil;9548:Matzingen;9552:Bronschhofen;9553:Bettwiesen;9554:Tägerschen;9555:Tobel;9556:Affeltrangen,Zezikon;9562:Märwil,Buch b. Märwil;9565:Bussnang,Oberbussnang,Oppikon,Schmidshof,Rothenhausen;9573:Littenheid;9601:Lütisburg Station;9602:Bazenheid,Müselbach;9604:Lütisburg,Oberrindal,Unterrindal;9606:Bütschwil;9607:Mosnang;9608:Ganterschwil;9612:Dreien;9613:Mühlrüti;9614:Libingen;9615:Dietfurt;9620:Lichtensteig;9621:Oberhelfenschwil;9622:Krinau;9630:Wattwil;9631:Ulisbach;9633:Hemberg,Bächli (Hemberg);9642:Ebnat-Kappel;9643:Krummenau;9650:Nesslau;9651:Ennetbühl;9652:Neu St. Johann;9655:Stein SG;9656:Alt St. Johann;9657:Unterwasser;9658:Wildhaus"

var _npaMappa = null      // NPA -> [localita]
var _locMappa = null      // localita (minuscolo) -> [NPA]

function npaIndice() {
  if (_npaMappa) return true
  try {
    _npaMappa = {}
    _locMappa = {}
    var rec = NPA_CH.split(';')
    for (var i = 0; i < rec.length; i++) {
      ...
      for (var k = 0; k < loc.length; k++) {
        var chiave = loc[k].toLowerCase()
        if (!_locMappa[chiave]) _locMappa[chiave] = []
        if (_locMappa[chiave].indexOf(npa) === -1) _locMappa[chiave].push(npa)
      }
    }
    return true
  } catch (e) {
    console.warn('Elenco NPA non utilizzabile:', e.message || e)
    _npaMappa = {}; _locMappa = {}
    return false
  }
}

function localitaDiNpa(npa) {
  if (!npaIndice()) return []
  return _npaMappa[String(npa || '').trim()] || []
}

function npaDiLocalita(loc) {
  if (!npaIndice()) return []
  return _locMappa[String(loc || '').trim().toLowerCase()] || []
}

// ── L'aggancio su una coppia di campi ───────────────────────────────────────
// `campi` = { npa, citta, paese, suggerimenti }: gli id degli elementi.
// `paese` puo' mancare (allora si assume CH: e' il caso delle Impostazioni
// ditta, dove il paese sta in un altro punto della pagina).

function npaAttivo(campi) {
  if (!campi.paese) return true
  var e = el(campi.paese)
  if (!e) return true
  var v = String(e.value || '').trim().toUpperCase()
  return v === '' || v === 'CH'
}

function pulisciSuggerimentiNpa(campi) {
  if (campi.suggerimenti && el(campi.suggerimenti)) html(campi.suggerimenti, '')
}

// Il pannellino di scelta. Icone sempre con l'etichetta scritta.
function mostraSceltaNpa(campi, titolo, voci, campoDaRiempire) {
  if (!campi.suggerimenti || !el(campi.suggerimenti)) return
  var bottoni = voci.map(function (v) {
    return '<button type="button" class="npa-scelta" ' +
      'onclick="applicaSceltaNpa(\'' + esc(campoDaRiempire) + '\', \'' + esc(v) + '\', \'' +
      esc(campi.suggerimenti) + '\')">' + esc(v) + '</button>'
  }).join('')
  html(campi.suggerimenti,
    '<div class="npa-suggest"><span class="npa-suggest-et">' + esc(titolo) + '</span>' +
      bottoni + '</div>')
}

function applicaSceltaNpa(idCampo, valore, idSuggerimenti) {
  var e = el(idCampo)
  if (e) { e.value = valore; e.dispatchEvent(new Event('change', { bubbles: true })) }
  if (el(idSuggerimenti)) html(idSuggerimenti, '')
}

// ── NPA scritto → la localita' ──────────────────────────────────────────────
function onNpaScritto(campi) {
  pulisciSuggerimentiNpa(campi)
  if (!npaAttivo(campi)) return
  var eNpa = el(campi.npa), eCitta = el(campi.citta)
  if (!eNpa || !eCitta) return
  var npa = String(eNpa.value || '').trim()
  if (!/^\d{4}$/.test(npa)) return          // non ancora completo, o estero: zitto
  var loc = localitaDiNpa(npa)
  if (!loc.length) return                    // non in elenco: nessun avviso, si scrive lo stesso

  // La regola che vale piu' di tutte: non si tocca quello che c'e' gia'.
  if (String(eCitta.value || '').trim()) return

  if (loc.length === 1) {
    eCitta.value = loc[0]
    eCitta.dispatchEvent(new Event('change', { bubbles: true }))
  } else {
    mostraSceltaNpa(campi, 'Per il ' + npa + ' scegli la località:', loc, campi.citta)
  }
}

// ── Localita' scritta → il NPA ──────────────────────────────────────────────
function onLocalitaScritta(campi) {
  pulisciSuggerimentiNpa(campi)
  if (!npaAttivo(campi)) return
  var eNpa = el(campi.npa), eCitta = el(campi.citta)
  if (!eNpa || !eCitta) return
  var nome = String(eCitta.value || '').trim()
  if (nome.length < 3) return
  var npa = npaDiLocalita(nome)
  if (!npa.length) return
  if (String(eNpa.value || '').trim()) return   // gia' scritto: non si tocca

  if (npa.length === 1) {
    eNpa.value = npa[0]
    eNpa.dispatchEvent(new Event('change', { bubbles: true }))
  } else {
    // Qui l'ambiguita' e' la norma: una citta' ha piu' NPA, e sceglierne uno
    // a caso vorrebbe dire scrivere un indirizzo sbagliato.
    mostraSceltaNpa(campi, 'Per «' + nome + '» ci sono più NPA:', npa, campi.npa)
  }
}

// ── Installazione ───────────────────────────────────────────────────────────
// Si chiama una volta per coppia di campi. `input` e non `change`: il
// completamento deve arrivare mentre si scrive, non quando si esce dal campo.
var _npaInstallati = {}

function installaNpaLocalita(campi) {
  var chiave = campi.npa + '|' + campi.citta
  if (_npaInstallati[chiave]) return
  var eNpa = el(campi.npa), eCitta = el(campi.citta)
  if (!eNpa || !eCitta) return
  _npaInstallati[chiave] = true
  eNpa.addEventListener('input', function () { onNpaScritto(campi) })
  eCitta.addEventListener('input', function () { onLocalitaScritta(campi) })
  // Cambiando paese i suggerimenti aperti non hanno piu' senso.
  if (campi.paese && el(campi.paese)) {
    el(campi.paese).addEventListener('change', function () { pulisciSuggerimentiNpa(campi) })
  }
}

// Le due coppie che esistono nel programma. Sono due: in fattura di vendita
// l'indirizzo e' un campo unico, e sulla busta e' una textarea.
var COPPIE_NPA = [
  { npa: 'c-cap',   citta: 'c-citta',   paese: 'c-paese',   suggerimenti: 'c-npa-suggest' },
  { npa: 'imp-npa', citta: 'imp-citta', paese: 'imp-paese', suggerimenti: 'imp-npa-suggest' }
]

function installaTutteLeCoppieNpa() {
  for (var i = 0; i < COPPIE_NPA.length; i++) installaNpaLocalita(COPPIE_NPA[i])
}
...
async function scegliContatto(prefix, id) {
  ...
  // FASE 27 — sulla fattura di vendita il termine si propone dal cliente
  // appena scelto, MA solo se Umberto non l'ha gia' cambiato a mano su questa
  ...
  if (prefix === 'v' && el('f-fat-giorni') && !terminiFatturaToccati) {
    ...
    if (gCli != null) {
      el('f-fat-giorni').value = Math.round(gCli)
      ricalcolaScadenzaDaGiorni()
      compilati.push('termine ' + testoTerminePagamento(gCli).toLowerCase())
    }
  }
  ...
  try {
    ...
    if (troppoAlta) {
      showFattureBanner('fatture-detail-banner', 'warn',
        ...
        'in stampa il fondo resterà fuori. Non la rimpicciolisco — una polizza scalata non è ' +
        'più valida. Se puoi, riscarica dalla banca la sola parte pagamento.')
    }
    return true
  } catch (e) {
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // FASE 27 — NPA e localita' che si compilano a vicenda. Si aggancia una
  // volta sola: i campi esistono gia' nell'HTML, non nascono dopo.
  installaTutteLeCoppieNpa()
